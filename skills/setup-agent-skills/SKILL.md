...
**读取**：编写代码或实施计划前，扫描 `docs/adr/` 标题列表，按当前任务关键词匹配加载相关 ADR（不要全部读取）。ADR 与当前代码冲突时以代码为准并标记冲突。目录不存在时静默继续。

**提炼**：用户在会话中指出修正时，判断是否揭示了可复现的模式性问题。是则主动提议创建 ADR（给出标题和一句话摘要），经确认后写入 `docs/adr/`。一次性笔误或已有 ADR 覆盖的不重复提。
```
