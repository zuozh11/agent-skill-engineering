...
如果需求里已经给出经过评审确认的代码片段或编排顺序，任务卡要把它们当成实施契约写死，而不是只当参考示意；任务只允许补全真实字段名、真实方法签名、边界条件和验收口径，不允许把已确认的业务语义重新泛化。
...
- 如果任务需求里已有评审后的代码片段，任务卡里的代码块必须与其逻辑和编排一致；只允许为了真实 API、字段名或可编译性做最小必要调整，不得自行扩大过滤条件、重排状态流转或改写对象复制范围
