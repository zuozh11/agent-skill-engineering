...
PRD 默认采用前后端同时交付视角。只有用户明确要求单端交付时，才改用纯前端或纯后端视角。
...
默认采用前后端同时交付视角，并按以下结构明确分开两侧职责：

- **Scope**：分别使用「前端范围」和「后端范围」列出两侧交付内容。
- **Requirements**：先按业务能力分节；同时涉及两端的能力在节内分别使用「前端逻辑」和「后端逻辑」，只涉及单端的能力只保留对应小节。
- **前端逻辑**：写清页面流转、页面状态、字段显隐、表单校验、操作反馈和接口调用顺序。
- **后端逻辑**：写清业务规则、状态流转、数据变化、查询隔离、权限边界、接口契约和流程回写。

只有用户明确要求纯前端或纯后端交付时，才采用对应的单端视角，并只保留该侧结构。

书写视角必须贯彻到 Scope、User Stories、Requirements 和 Implementation Decisions。前后端视角下不得把两侧逻辑混写在同一组笼统条目中。
...
#### 前端逻辑
- <页面行为、字段显隐、表单校验、调用顺序和操作反馈>

#### 后端逻辑
- <业务规则、状态、权限、数据变化、接口契约和流程回写>
...
- 检查整份 PRD 是否始终贯彻所选视角：默认前后端视角下，Scope 必须分为前端范围和后端范围；每个同时涉及两端的 Requirements 能力项必须分别包含前端逻辑和后端逻辑
- 前端逻辑只描述交互、展示、校验和调用行为，后端逻辑只描述服务端规则、数据和流程行为；不得在同一条目中混写两侧职责
