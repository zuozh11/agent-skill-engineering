---
name: to-prd
description: 将已讨论的需求整理为 PRD；用户要求生成或更新需求文档时使用。
---
...
先复用已确认决策，按「写法」判断是否足以形成完整可评审的 PRD。没有关键未决取舍时直接成稿。

已锁定决策来自本轮用户确认、已有 PRD 的 Implementation Decisions、对应 `WAYFINDER.md` 的 Decisions so far 及已关闭决策票 Resolution。仅当尚未锁定的取舍会改变实现分支、数据口径、状态变化、权限范围或接口契约时，围绕这些问题调用 `ask-me`，并传入相关已锁定决策。

等待决策时继续整理不依赖答案的内容。未决业务规则明确标注，不自行补成已确认要求；关键取舍解决后继续成稿，不在 `ask-me` 返回处结束任务。
