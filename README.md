...
- **`docs/guides/`** — 编码指南。记录代码实现时要遵守的约束、模式和团队偏好，供 `/to-task`、`/impl`、`/retro` 按需读取。

`CONTEXT.md` 和 `docs/adr/` 由 `/grill-with-docs` 在对话中按需创建和更新；`docs/guides/` 由 `/setup-agent-skills` 初始化，后续可由 `/retro` 在复盘后补充。
