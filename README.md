...
**解法**：`/impl` 根据实现范围、工作区状态和隔离收益，自动选择当前工作区或独立 worktree 实现；需要直接指定隔离模式时使用 `/impl -w`。两种方式都保持原子提交和任务状态流转。
...
| **[impl](./skills/impl/SKILL.md)** | **根据实现范围、工作区状态和隔离收益自动选择执行方式，支持用 `-w` 强制 worktree 隔离实现** |
