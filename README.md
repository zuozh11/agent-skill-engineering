...
**解法**：`/impl` 按依赖和写集拆分多张任务卡，每张已解锁任务卡必须在独立的全新执行上下文中实施；上下文隔离不决定 worktree、执行者或串并行策略。执行时根据实现范围、工作区状态和隔离收益自动选择当前工作区或独立 worktree；需要直接指定 worktree 时使用 `/impl -w`。完成验证并检查 staged diff 后原子提交；需要代码评审时单独调用 `/code-review`。
