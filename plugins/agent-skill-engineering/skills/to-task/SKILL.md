...
切分完成后，每项已确认需求都应当能在某张卡中找到归属；每张卡也应当能够独立说明要交付的业务结果。

### 3. 写入任务卡

用户指定输出目录时直接使用；已有对应需求目录时在该目录下创建 `tasks/`。否则创建 `docs/scratch/<下一个两位序号>-<中文需求名称>/tasks/`。中文需求名称从需求主题推导并沿用需求上下文中的术语。
