...
使用简洁业务语言，但架构名词严格来自 `codebase-design`：**模块、接口、实现、深度、深、浅、接缝、适配器、杠杆、局部性**。
...
- “定价逻辑泄漏到了接缝之外。”
- “深化：一个接口，一个测试位置。”
- “两个适配器证明接缝真实存在：生产 HTTP，测试内存实现。”

收益条目直接说结果，例如“局部性：缺陷集中在一个模块”“杠杆：一个接口服务多个调用者”。避免“更易维护”“代码更干净”等无法说明具体收益的笼统表述。
