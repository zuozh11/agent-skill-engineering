...
运行时 `scope` 只返回 Map 注册的稳定 `path` 和对应 `description`，并以单行 JSON 输出；`scope --compact` 是等价兼容入口，`scope --pretty` 只美化相同数据。Agent 将所选 `path` 作为可重复的 `load --context` 参数。`load --compact` 用 `## CONTEXT <description>` 替代 Context Frontmatter 和重复一级标题；多 Context 项目的 Map 保留共享概念、Relationships 与其他正文，但省略已由 `scope` 提供的 `## Contexts` 发现列表。完整 `load` 保留原始诊断信息。
