...
每个 RULE 只表达一个可独立判断的原子约束，正文必须为一到三句话，每句话单独占一个非空行；Frontmatter 和一级标题不计入句数。
...
迁移长 RULE 时，先列出原约束，再压缩或拆分为多个原子 RULE；拆出的每个文件都使用 Frontmatter，每句话独占一行，并用相对 `references` 保留原有依赖和语义。迁移前后逐项核对，不得因压缩丢失例外或验收含义。
