...
- 英文场景编码只使用大写英文字母，位数不限，不携带固定含义或顺序。
- 场景内编码固定为两位数字，只要求完整编码不重复。
...
2. 场景内编号选择任一未使用的两位数字 `00`–`99`；不要求连续，也不表达优先级。
3. 新增场景时由用户确认新的大写英文 code 与场景名称；code 不编码语义，也不要求接续现有顺序。
...
- RULE 场景按 code 的 UTF-8 字节序输出；场景内规则先按两位数字的数值升序，再以完整文件名 UTF-8 字节序作为确定性兜底。顺序不表达优先级。
